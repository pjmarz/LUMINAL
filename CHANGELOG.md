...
- ~~Pre-existing drift~~ (superseded by the web-search Fixed entry above — engine is now searxng with a populated query URL): `webui.db` had `web.search.engine = "perplexity"` with a real Perplexity key and an empty `web.search.searxng_query_url`, while the compose file still declares `WEB_SEARCH_ENGINE=searxng` + `SEARXNG_QUERY_URL`. The DB wins, so the `searxng` service added in 1.6.0 is currently unused.
